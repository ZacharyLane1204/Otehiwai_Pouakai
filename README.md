...
**Use Python 3.11 (the `environment.yml` default), not 3.9**, unless you
have a specific reason to match an old 3.9.7 deployment exactly:
`astroquery`'s GitHub `main` branch (installed by default -- see below)
has dropped support for Python <3.10, so `pip install -e .` will fail
with `Package 'astroquery' requires a different Python: 3.9.7 not in
'>=3.10'` on a 3.9 env. If you genuinely need 3.9, pin `astroquery` to a
released version instead of `main` in `pyproject.toml` (swap the
`astroquery @ git+...` line for e.g. `"astroquery>=0.4.6,<0.5"`) rather
than changing the Python version.
...
(a plain `pip install -e .` shouldn't silently reach out to GitHub and
build a third-party package). Install it explicitly, **from inside the
repo directory, referencing the local path (`.`) -- not the package
name** (this package is not published to PyPI, so `pip install
"otehiwai-pouakai[...]"` by name alone will always fail with "No
matching distribution found"):

```bash
cd Otehiwai_Pouakai        # repo root -- the `.` below means "this checkout"
...
- **`astroquery` (installed from source) needs Python >=3.10.** If
  ...
  -- use the `environment.yml` default of Python 3.11 instead (see
  Install, Option A), or pin `astroquery` to a released PyPI version
  rather than `main` if you specifically need 3.9 (see the comment next
  to that dependency line in `pyproject.toml`).
